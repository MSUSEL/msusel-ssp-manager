control 'app_opa_integration' do
  ...
  app_protected_url = 'http://localhost:8000/protected'
  ...
  describe file(log_file_path) do
    its('content') { should include '"sub"' }
    its('content') { should include '"testuser"' }
    its('content') { should include '"allow"' }
    its('content') { should include 'true' }
  end
end
