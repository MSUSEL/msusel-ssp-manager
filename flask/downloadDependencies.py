...
def extractSourceCode(name):
    ...
    filesList = os.listdir('dependencies/' + name)
    file = ''
    if filesList != []:
        file = filesList[0]
    ...
    if file.endswith('.zip'):
        ...
        with zipfile.ZipFile('dependencies/' + name + '/' + file, 'r') as zip_ref:
            zip_ref.extractall('dependencies/' + name)
    # Check if the file is a tar file
    elif file.endswith('.tar.gz'):
        # Extract the source code from the tar file
        with tarfile.open('dependencies/' + name + '/' + file, 'r:gz') as tar_ref:
            tar_ref.extractall('dependencies/' + name)
    # Check if the file is a tar file
    elif file.endswith('.tar'):
        # Extract the source code from the tar file
        with tarfile.open('dependencies/' + name + '/' + file, 'r') as tar_ref:
            tar_ref.extractall('dependencies/' + name)
    # Check if the file is a tar file
    ...
    # Check if the file is a tar file
        ...
        # Extract the source code from the tar file
        with tarfile.open('dependencies/' + name + '/' + file, 'r:bz2') as tar_ref:
            tar_ref.extractall('dependencies/' + name)
    # Check if the file is a tar file
    elif file.endswith('.tar.xz'):
        # Extract the source code from the tar file
        with tarfile.open('dependencies/' + name + '/' + file, 'r:xz') as tar_ref:
            tar_ref.extractall('dependencies/' + name)
    # Check if the file is a tar file
    elif file.endswith('.tar.Z'):
        # Extract the source code from the tar file
        with tarfile.open('dependencies/' + name + '/' + file, 'r:Z') as tar_ref:
            tar_ref.extractall('dependencies/' + name)
    # Check if the file is a tar file
    elif file.endswith('.tar.lz'):
        # Extract the source code from the tar file
        with tarfile.open('dependencies/' + name + '/' + file, 'r:lz') as tar_ref:
            tar_ref.extractall('dependencies/' + name)
    # Check if the file is a tar file
    ...
    # Check if the file is a tar file
        ...
        # Extract the source code from the tar file
        with tarfile.open('dependencies/' + name + '/' + file, 'r:lzo') as tar_ref:
            tar_ref.extractall('dependencies/' + name)
    # Check if the file is a tar file
    elif file.endswith('.tar.Z'):
        # Extract the source code from the tar file
...
def downloadSourceCode(name, version):
    if version != '':
        ...
        subprocess.run(['pip', 'download', '--no-binary', ':all:', '--no-deps', '-d', 'dependencies/' + name, name + '==' + version])
    else:
        subprocess.run(['pip', 'download', '--no-binary', ':all:', '--no-deps', '-d', 'dependencies/' + name, name])
    # Extract the source code from the downloaded file
    extractSourceCode(name)
    print('Downloaded the source code for ' + name + ' ' + version)
...
def download_Dependencies():
    remove_bom_from_file('./artifacts/dependencies.txt')
    # Read the dependencies names and versions from the text file
    with open('./artifacts/dependencies.txt', 'r') as file:
    ...
    for dep in dependencies:
        ...
        if not os.path.exists('dependencies/' + name):
            os.makedirs('dependencies/' + name)
        print('Downloading the source code for ' + name + ' ' + version)
        # Download the source code for the dependency
        downloadSourceCode(name, version)
    print('Downloaded the source code for the dependencies.')
